...
export default function GemmaChat() {
  const { tickData, externalAlerts } = useCrisisStore();
  const alerts = [...(tickData?.alerts || []), ...externalAlerts];
  const [isOpen, setIsOpen] = useState(false);
  const [status, setStatus] = useState<'checking' | 'online' | 'offline' | 'loading'>('checking');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const activeAlerts = alerts.slice(-10); // Use the 10 most recent alerts for context
